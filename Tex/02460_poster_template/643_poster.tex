...
\begin{itemize}
\item It is not feasible to use all of the available data.
\item There is a high redundancy between datapoints (25 fps).
\item Time taken in fitting a model is rarely linear to the input.
\end{itemize}
...
\item These scores are normalised into a distribution \H  to sample from.
\end{enumerate}
Ma. et al. \cite{Ma} show that the leverage-scores for least-square regression is the diagonal elements of \H
\begin{equation}
\H = (\X^T \X)^{-1}\y \label{eq:hdist}
\end{equation}
...
\caption{LS- vs. uniform-distribution based sampling for logistic regression. }
\end{figure}	
\begin{itemize}
\item A LS-distribution sample scheme is no better that uniform sampling on datasets of type GA and T3.
...
Using the LS-based sampling for classification shows know improvement on datasets \emph{GA} and \emph{T3}, but for \emph{T1} with very non-uniform leverage-scores, the approach is slightly better.
